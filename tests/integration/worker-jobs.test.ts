...
describe("YouTube upload worker", () => {
  it("downloads the rendered video and uploads it with channel credentials", async () => {
    ...
    expect(database.reserveUploadedVideosUsage).toHaveBeenCalledWith(
      workspaceId,
    );
    ...
    expect(database.incrementUploadedVideosUsage).not.toHaveBeenCalled();
  });

  it("reserves monthly upload capacity before calling YouTube", async () => {
    const video = new Uint8Array([1, 2, 3]);
    const database = makeDatabaseService({
      getYoutubeUploadContext: vi.fn().mockResolvedValue({
        description: "Generated by Bussin",
      ...
      reserveUploadedVideosUsage: vi
        .fn()
        .mockRejectedValue(new Error("trial allows 10 monthly uploads.")),
    });
    const storage: WorkerStorageService = {
      copyAudioFromUrl: vi.fn(),
      copyCoverFromUrl: vi.fn(),
    ...
    ).rejects.toThrow(/trial allows 10 monthly uploads/);
...
function makeDatabaseService(
  ...
  return {
    ...
    reserveUploadedVideosUsage: vi.fn().mockResolvedValue(undefined),
